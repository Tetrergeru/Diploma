...
\markboth{{\small\rm \hfill O.V. Arutyunov, V.A. Skorokhodov
...
\cite{algebraic_aproach_to_sc_ldpc, imrich_construction, lifts_of_graphs, Kim2002ExplicitCO, margulis_critique, margulis_construction, vasic_combinatorial_construction}.

Protographs are Tanner graphs, typically with relatively small number of vertices. They serve as blueprints for constructing larger LDPC-codes. A larger graph is obtained by "copy-and-permutate" operation: the first protograph is copied several times, after that endpoints of edges are swapped so that subgraphs become interconnected. The performance of the larger graph can be predicted by analyzing the protograph. 

Parts in Tanner graph are called subcode part and digit part respectively. Digit vertices correspond to bits passed through the channel, and subcode vertices correspond to parity checks of adjacent variable vertices. LDPC Algorithm checks values of subcode vertices, and if an error is detected, recursively traverses adjacent vertices. Thus if error has occurred in vertex belonging to a cycle, it will accumulate when the algorithm fully walks the cycle. Hence graphs without short cycles should be used.

In this article, we propose a structural method for construction of bipartite graphs with any given girth by expanding metagraphs. The algorithm for selection of optimal weights for metagraphs is developed. Precise estimation of girth among all expansions of metagraph is given. An algorithm for iterative selection of metragraph structure is proposed.
...
\begin{notice}
    Sometimes the distinction between walks, trails and paths is made, in this paper we decided to use word the path for all such objects.
\end{notice}
...
\begin{definition}
    Let $G$ be a graph. The girth of $G$ is the length of its shortest cycle.
\end{definition}
...
Example \ref{image:2} shows a metagraph  $G_1$ with three vertices and four edges.
...
\begin{figure}[!h]
    ...
    \begin{picture}(100,110)
        ...
        \put(59,45){$0$}
    \end{picture}
    \caption{ The metagraph with edges weights of +1, 0, 0, -1. }
    \label{image:2}
\end{figure}
...
Example \ref{image:3} shows a graph built as a 4-expansion of the metagraph $G_1$, shown in the example \ref{image:2}.
...
\begin{figure}[!h]
    ...
    \end{picture}
    \caption{ 4-expansion of the metagraph $G_1$. }
    \label{image:3}
\end{figure}
...
\begin{notice}
    If some path is a cycle on metagraph $G$, then its corresponding paths on $G^{(r)}$ might not be cycles. Such a case is demonstrated by the following example.
\end{notice}

\textbf{Example 1.}

Consider a graph in figure \ref{metagraph_3_expansion}. Path $(e_1, e_2)$ is a cycle on metagraph but none of its corresponding paths  $\mu_1 = (e^{(1)}_1, e^{(1)}_2)$, $\mu_2 = (e^{(2)}_1, e^{(2)}_2)$, $\mu_3 = (e^{(3)}_1, e^{(3)}_2)$  on 3-expansion is a cycle. Notice that concatenation of these three paths $\mu = (e^{(1)}_1, e^{(1)}_2, e^{(2)}_1, e^{(2)}_2, e^{(3)}_1, e^{(3)}_2)$ is a cycle with length 6. Cycle $\mu$ corresponds to cycle $(e_1, e_2, e_1, e_2, e_1, e_2)$ on metagraph.
...
\begin{theorem}[on last vertices of paths]\label{theorem_last_vertices}
    Let $\eta = (e_1, ..., e_d)$ be a path on metagraph $G$ with first vertex $x$ and last vertex $y$. Let $\mu' = (e'_1, ..., e'_d)$ be one of its corresponding paths on $G^{(r)}$ with first vertex $x^{(i)}$.
...
\begin{proof}
    ...
    Suppose $\eta' = (e_1, ..., e_{d - 1})$ is a path. Let us call $x$ the last vertex of path $\eta$. According to the supposition of induction the last vertices of its corresponding paths on expansion are $x^{(i + \chi(\eta')(\mod{r}))}$.
...
\begin{theorem}[on cycles]\label{theorem_cycles}
    Let $\eta$ be a cycle on metagaph. Then its corresponding paths on the expansion are cycles iff $\chi(\eta) = 0 (\mod{r})$.
\end{theorem}
...
Let $G(V, E, f, w)$ be a metagraph and $w(e_i) = w_i \; \forall i = 0, \dots, |E|$, where $w_i$ are variables that will help us build a graph with a given girth.
...
Let $G$ be a metagraph. Then there exists an expansion of metagraph $G$ with a girth equal to or greater than $g$ iff the following system of inequalities for a graph $G$ has a solution:
...
Assigning $w_3 = 0$ in \eqref{eqs:cycle_search_neqs_2} similarly, we get
...
Variable $w_4$ can not be equal to $0, 1$ or $-1$ since they violate the last inequalities. Assign $w_4 = 2$ in \eqref{eqs:cycle_search_neqs_3}. We finally obtain
...
Note that system \eqref{eqs:cycle_search_neqs_4} does not allow $r$ to be neither $1, 2, 3$ nor $4$, thus the least possible value of $r$ is $5$. 5-expansion of metagraph $G$, considered in figure \ref{neq_system_graph} with assigned weights, is shown in figure \ref{neq_system_res}. Girth of this graph is equal to $6$, and one of the cycles with length $6$ is marked in bold.
...
In previous section we showed a method of finding weights that allows us to build metagraph expansions with given girth.
But we have not suggested a way to select a structure of a metagraph such that it would be possible to find such weights, i.e. such that system \eqref{eqs:example} built by Algorithm 1 is non-contradicting.

% Какое-то странное замечание
\begin{notice}
    Let us sequentially apply Algorithm 1 to metagraph $G$ with target girths of $4, 6, \dots, 2n$, where $2n$ is the first number for which algorithm couldn't build a solution. Then maximal girth of metagraph $G$ expansions is $2n - 2$.
\end{notice}
...
If a cycle on metagraph corresponds to cycles on all expansions, then its characteristic is $0$ independently of the selected weights. Hence this cycle walks through each of its edges the same number of times in the opposite directions.
...
{\small {\sc A. Beemer, Habib, S., Kelley, C. A., Kliewer, J.: }{\it A generalized algebraic approach to optimizing SC-LDPC codes.} In 55th Annual Allerton Conference on Communication, Control, and Computing, Allerton 2017 (pp. 672-679).Institute of Electrical and Electronics Engineers Inc. doi:10.1109/ALLERTON.2017.8262802 }
...
{\small {\sc W. Imrich: }{\it Explicit construction of regular graphs without small cycles.} Combinatorica, 4(1), 53–59. (1984) doi:10.1007/bf02579157 } 
%% https://sci-hub.se/10.1007/bf02579157

% 2
\bibitem{johnson}
{\small {\sc S. Johnson: }{\it Introducing Low-Density Parity-Check Codes.} 2008.}
...
{\small {\sc J.-L. Kim, N.P. Uri , I.G. Perepelitsa, V. Pless: }{\it “Explicit Construction of Families of LDPC Codes with Girth at Least Six.} Proceedings of the 40th Allerton Conference on Communication, Control and  Computing, UIUC, Oct. 2002}

% 8
%% Критика метода Маргулиса
\bibitem{margulis_critique}
{\small {\sc D.J.C. MacKay, M.S. Postol: }{\it Weaknesses of Margulis and Ramanujan-Margulis Low-Density Parity-Check Codes.} Electronic Notes in Theoretical Computer Science, Volume 74, 2003, Pages 97-104, ISSN 1571-0661, doi:10.1016/S1571-0661(04)80768-0}
% https://www.inference.org.uk/mackay/margulis.pdf

% 7
\bibitem{margulis_construction}
{\small {\sc G. A. Margulis: }{\it Explicit constructions of graphs without short cycles and low density codes.} Combinatorica, 2(1), 71–78. (1982) doi:10.1007/bf02579283}
%% https://sci-hub.se/10.1007/bf02579283

% 10
\bibitem{ldpc_encoding}
{\small {\sc T. J. Richardson, R. L. Urbanke: }{\it Efficient encoding of low-density parity-check codes.} IEEE Transactions on Information Theory, vol. 47, no. 2, pp. 638-656, Feb 2001, doi: 10.1109/18.910579 }

% 5
\bibitem{skorohodov_reachability_problem}
{\small {\sc V.A. Skorokhodov: }{\it Generalization of the Reachability Problem on Directed Graphs.} Mathematics and Statistics, Vol. 8, No. 6, pp. 699 - 704, 2020. DOI: 10.13189/ms.2020.080610.}
...
{\small {\sc B. Vasic, O. Milenkovic: }{\it Combinatorial constructions of low-density parity-check codes for iterative decoding.} in IEEE Transactions on Information Theory, vol. 50, no. 6, pp. 1156-1176, June 2004, doi: 10.1109/TIT.2004.828066. }
