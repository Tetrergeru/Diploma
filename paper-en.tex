...
%% Проверить, что оно нигде дальше не используется
% \begin{definition}
...
\begin{theorem}
    ...
    Let $G(V, E, f, w)$ be a metagraph, let $\mu$ be a cycle on metagraph $G$, let $a$ and $b$ be two (possibly equal) vertices on cycle $\mu$, and let $\eta$ be a path from $a$ to $b$ such that the first and the last edges of $\eta$ do not belong to $\mu$. Then girth of any r-expansion of $G$ is not greater than $2(|\mu| + |\eta|)$.

\end{theorem}

\begin{proof}

    Denote inverse paths of $\mu$ and $\eta$ as $\mu'$ and $\eta'$ respectively.
    ...
    Thus corresponding paths of $\nu$ are cycles on any r-expansion of $G$. This means that girth of any r-expansion of $G$ is at most $2(|\mu| + |\eta|)$.

\end{proof}

% Становится полноценной теоремой, которую надо доказать

\begin{theorem}\label{theorem_cycle_expansion}

    Let $G(V, E, f)$ be a bipartite graph with girth $2l$. Let $G'(V, E, f, w')$ be a metagraph that is a weighted graph $G$. Then there is an r-expansion of $G'$ with girth greater then or equal to $6l$.
